...
pTextBrush := Gdip_CreateLineBrushFromRect(0, Integer(RC[2]), WinWidth, Integer(RC[4]), 0xffDDF7F7, 0x774549DF)
...
Gdip_SetClipRect(G, Integer(RC[1]), Integer(RC[2]), Integer(RC[3]), Integer(RC[4]))
...
BRA_GetCount(&BRAFromMemIn) {
	...
	InfoLength := (SubStr(A_AhkVersion,1,1) < 2) ? Info.Length() : Info.Length
	If (InfoLength != 3)
		Return -3
	Return (Info[1] + 0)
}
...
Fade(InOut)
{
	...
	numLoops := 0.8/0.05
	N := (SubStr(A_AhkVersion,1,1) < 2) ? numLoops : "numLoops"
	Loop %N%
	{
        ...
        ; msgbox WinWidth " / " NewWidth " / " NewHeight
        
		Gdip_SetClipRect(G, Integer(WinWidth-NewWidth)//2, Integer(WinWidth-NewWidth)//2, Integer(NewWidth), Integer(NewHeight))
		...
		Gdip_DrawImage(G, pBitmap, Integer(WinWidth-NewWidth)//2, Integer(WinWidth-NewWidth)//2, NewWidth, NewHeight, 0, 0, Width, Height, Trans)
...
else
{
	...
	Gdip_SetClipRect(G, Integer(WinWidth-NewWidth)//2, Integer(WinWidth-NewWidth)//2, NewWidth, NewHeight)
	Gdip_SetCompositingMode(G, 1)
	Gdip_FillRectangle(G, pBrush, 0, 0, WinWidth, WinHeight)
	Gdip_SetCompositingMode(G, 0)
	...
	Index++
	pBitmap := Gdip_BitmapFromBRA(&BRA, Index, 1)
	Gdip_DrawImage(G, pBitmap, Integer(WinWidth-NewWidth)//2, Integer(WinWidth-NewWidth)//2, NewWidth, NewHeight, 0, 0, Width, Height, 0.8)
	UpdateLayeredWindow(hwnd1, hdc)
	Gdip_DisposeImage(pBitmap)
	Gdip_ResetClip(G)
}
